...
class UpdatePatientProfileRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $profile = \App\Models\PatientProfile::find($profileId);
            if (!$profile) return;

            if ($this->input('is_self') && (!$profile->is_self || $profile->owner_id != $this->input('owner_id'))) {
                $hasSelf = \App\Models\PatientProfile::where('owner_id', $this->input('owner_id'))
                            ->where('is_self', 1)
                            ->where('id', '!=', $profileId)
                            ->exists();
                if ($hasSelf) {
                    $validator->errors()->add('is_self', 'Khách hàng này đã có hồ sơ bản thân. Vui lòng chọn loại hồ sơ là "Người thân".');
                }
            }
        });
    }
