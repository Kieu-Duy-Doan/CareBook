    ...
    public function collection(Collection $rows)
    {
        ...
        DB::transaction(function () use ($rows) {
            foreach ($rows as $index => $row) {
                ...
                $id             = trim($row[0] ?? '');
                $doctorCode     = trim($row[1] ?? '');
                $roomName       = trim($row[3] ?? '');
                $dayOfWeekInput = trim($row[4] ?? '');
                $shiftInput     = trim($row[5] ?? '');
                $duration       = trim($row[8] ?? '');
                $maxSlots       = trim($row[9] ?? '');
                ...
                $existsTimeQuery = WorkSchedule::where('doctor_profile_id', $doctor->id)
                ...
                if ($id) {
                    $existsRoomOverlapQuery->where('id', '!=', $id);
                }
                if ($existsRoomOverlapQuery->exists()) {
