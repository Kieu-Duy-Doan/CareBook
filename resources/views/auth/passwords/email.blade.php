...
<body class="antialiased bg-slate-50 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-white rounded-2xl shadow p-8">
        ...
        <p class="text-sm text-slate-500 mb-6">Nhập số điện thoại đã đăng ký, chúng tôi sẽ gửi liên kết đặt lại mật khẩu tới email của bạn.</p>

        @if(session('error'))
            <div class="bg-rose-50 text-rose-700 p-3 rounded mb-4">{{ session('error') }}</div>
        @endif
        @if(session('success'))
            <div class="bg-emerald-50 text-emerald-700 p-3 rounded mb-4">{{ session('success') }}</div>
        @endif
        ...
        <form action="{{ route('password.email') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input name="phone" type="text" value="{{ old('phone') }}" required
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nhập số điện thoại">
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg">Gửi liên kết</button>
            </div>
        </form>
    </div>
</body>
