...
<div x-show="step === 4" class="max-w-5xl mx-auto px-4 py-6">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white border rounded-3xl p-6 shadow-sm" style="border-color:#e2e8f0;">
                ...
                <textarea x-model="reason"
                    ...
                    class="w-full border-2 border-gray-200 rounded-2xl px-5 py-4 resize-none text-gray-800 focus:outline-none focus:ring-0 focus:border-primary placeholder-gray-400 text-base"></textarea>
        ...
        <div>
            <div class="bg-white border-2 rounded-3xl overflow-hidden shadow-sm h-full flex flex-col" style="border-color:#e2e8f0;">
                ...
                <div class="divide-y divide-gray-100 flex-1">
                    ...
                    <div class="flex items-start px-5 py-3 hover:bg-slate-50 transition-colors">
                        ...
                        <span class="font-bold text-gray-800 text-base"
                            x-text="bookingMethod === 'suggested' ? 'Bác sĩ gợi ý' : (bookingMethod === 'doctor' ? 'Chỉ định bác sĩ' : 'Đặt lịch cơ bản')"></span>
                    </div>
                    <div class="flex items-start px-5 py-3 hover:bg-slate-50 transition-colors" x-show="bookingMethod === 'doctor' || bookingMethod === 'suggested'">
                    ...
                    <div class="flex items-start px-5 py-3 hover:bg-slate-50 transition-colors">
                        ...
                        <span class="font-bold text-gray-800 text-base"
                            x-text="selectedDate ? new Date(selectedDate.date || selectedDate).toLocaleDateString('vi-VN') : ''"></span>
                    </div>
                    <div class="flex items-start px-5 py-4 hover:bg-slate-50 transition-colors bg-primary/5">
                        ...
                        <span class="font-black text-xl" style="color:var(--primary);" x-text="selectedSlot?.time || selectedSlot"></span>
                    </div>
                    <div class="flex items-start px-5 py-3 hover:bg-slate-50 transition-colors">
                ...
                <div class="flex items-center px-5 py-4 bg-gray-50 mt-auto border-t" style="border-color:#e2e8f0;">
                    ...
                    <span class="ml-auto font-black text-lg" style="color:var(--primary);"
                        x-text="totalFee > 0 ? new Intl.NumberFormat('vi-VN', {style: 'currency', currency: 'VND'}).format(totalFee) : 'Liên hệ tại quầy'"></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('booking.store') }}" id="booking-form"
        ...
        <input type="hidden" name="specialty_id"
            :value="bookingMethod === 'suggested' ? oldAppointment?.specialty_id : selectedSpecialty?.id">
        ...
        <input type="hidden" name="appointment_date" :value="bookingMethod === 'suggested' && selectedDate && typeof selectedDate === 'string' ? selectedDate : selectedDate?.date">
        <input type="hidden" name="appointment_time" :value="selectedSlot?.time || selectedSlot">
        <input type="hidden" name="reason" :value="reason.trim() || 'Không có'">
        <input type="hidden" name="booking_method" :value="bookingMethod === 'suggested' ? (oldAppointment?.booking_method || 'doctor') : bookingMethod">
