        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <template x-for="profile in profiles" :key="profile.id">
                <div @click="selectProfile(profile)"
                     ...
                     :class="selectedProfile?.id === profile.id ? 'border-primary ring-1 ring-primary/20 bg-primary/5' : 'border-slate-200'">
                    ...
                    {{-- Avatar placeholder --}}
                    <div class="flex-shrink-0 w-10 h-10 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center ml-2 border border-blue-100">
            ...
            <a href="{{ route('patient.profiles.create') }}?redirect=booking"
               class="flex items-center justify-center gap-3 p-4 border-2 border-dashed border-slate-300 rounded-2xl transition-all duration-300 group hover:border-primary hover:bg-primary/5 bg-slate-50/50 min-h-[100px]">
                <div class="text-center">
                    <p class="font-bold text-slate-600 group-hover:text-primary transition-colors text-lg"><i class="fa-solid fa-plus-circle mr-1"></i> Thêm người mới</p>
                </div>
            </a>
        </div>
