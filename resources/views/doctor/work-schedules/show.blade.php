<x-layouts.doctor title="Chi tiết ca làm việc">
    <div class="mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8 pt-6 border-t border-gray-100">
            ...
            <div>
                ...
                <div class="font-semibold text-gray-900">
                    Tối đa: <span class="text-blue-600">{{ $schedule->max_slots ?? 2 }} slot</span><br>
                    @php
                        $start = \Carbon\Carbon::parse($schedule->start_time);
                        $end = \Carbon\Carbon::parse($schedule->end_time);
                        $duration = $schedule->slot_duration_minutes ?? 15;
                        $slotsCount = $duration > 0 ? intdiv($start->diffInMinutes($end), $duration) : 0;
                    @endphp
                    <span class="text-sm font-normal text-gray-500">
            ...
            <div>
                ...
                <div class="font-semibold text-gray-900 text-lg">
                    {{ $schedule->slot_duration_minutes ?? 15 }} phút
                </div>
            </div>
        </div>
    </div>
                        ...
                        @if (isset($weeklySchedules[$dayNum]) && count($weeklySchedules[$dayNum]) > 0)
                            @foreach ($weeklySchedules[$dayNum] as $ws)
                                ...
                                @if (!data_get($ws, 'is_override'))
                                    <a href="{{ route('doctor.work-schedules.show', data_get($ws, 'id')) }}"
                                ...
                                @else
                                    {{-- @php
                                        dd($ws);
                                    @endphp --}}
                                    <a href="{{ route('doctor.work-schedules.show', ['schedule' => data_get($ws, 'id'), 'type' => 'override']) }}"
                                        class="block bg-white border p-2.5 rounded shadow-sm hover:shadow-md transition-shadow cursor-pointer ">
                                        <div class="font-bold text-gray-900 text-xs flex items-center gap-1">
                                            <i class="fa-regular fa-clock text-gray-400"></i>
                                            {{ substr(data_get($ws, 'start_time'), 0, 5) }} -
