<x-layouts.doctor title="Thêm Hồ sơ bệnh án">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center text-sm text-gray-500 mb-2">
                ...
                <a href="{{ route('doctor.appointments.show', $appointment->id) }}" class="hover:text-blue-600 transition-colors">Chi tiết LH: {{ $appointment->appointment_code }}</a>
                <i class="fa-solid fa-chevron-right text-[10px] mx-2"></i>
                <span class="text-gray-800 font-medium">Thêm Hồ sơ bệnh án</span>
            </div>
            <h2 class="text-2xl font-bold text-gray-900">Thêm Hồ sơ bệnh án</h2>
        </div>
        <div>
    ...
    <form action="{{ route('doctor.medical-records.store', $appointment->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="md:col-span-2">
                ...
                <textarea name="diagnosis" rows="3" required class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none">{{ old('diagnosis') }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Mã ICD-10 (Tuỳ chọn)</label>
                <input type="text" name="icd10_code" value="{{ old('icd10_code') }}" class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none" placeholder="VD: J01.9">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Ngày tái khám (Tuỳ chọn)</label>
                <input type="date" name="followup_date" value="{{ old('followup_date') }}" class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none">
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Kết luận (Conclusion) (Tuỳ chọn)</label>
                <textarea name="conclusion" rows="2" class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none">{{ old('conclusion') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="treatment_result" required class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none">
                    ...
                    <option value="admitted" {{ old('treatment_result') === 'admitted' ? 'selected' : '' }}>Nhập viện (Admitted)</option>
                    <option value="monitoring" {{ old('treatment_result') === 'monitoring' ? 'selected' : '' }}>Theo dõi thêm (Monitoring)</option>
                </select>
            </div>
            <div class="md:col-span-2">
        ...
        <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-100">
            <a href="{{ route('doctor.appointments.show', $appointment->id) }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">Hủy</a>
            <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
                <i class="fa-solid fa-floppy-disk mr-2"></i> Lưu hồ sơ
            </button>
        </div>
    </form>
    ...
    <script>
        document.getElementById('result_files').addEventListener('change', function(e) {
            ...
            Array.from(this.files).forEach(file => {
                fileList.innerHTML += `<div class="flex items-center gap-2"><i class="fa-solid fa-file-pdf text-red-500"></i> <span>${file.name}</span></div>`;
            });
        });
    </script>
</x-layouts.doctor>
