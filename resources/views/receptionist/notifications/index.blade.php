<x-layouts.receptionist title="Thông báo">
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <form action="{{ route('receptionist.notifications.destroy-read') }}" method="POST"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
            ...
            <div class="p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-colors hover:bg-gray-50 {{ !$notif->is_read ? 'bg-emerald-50/30' : '' }}">
                <div class="flex items-start gap-4 flex-1">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 {{ in_array($notif->type, ['cancellation', 'system_cancellation']) ? 'bg-red-100 text-red-600' : ($notif->type === 'patient_cancellation' ? 'bg-gray-100 text-gray-600' : 'bg-emerald-100 text-emerald-600') }}">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            @if (!$notif->is_read)
                            <span class="px-2 py-0.5 bg-emerald-500 text-white text-[10px] font-bold rounded-full uppercase">Mới</span>
                            @endif
                        </div>
                ...
                <div class="flex items-center gap-1 sm:ml-4 self-end sm:self-auto shrink-0">
                    ...
                    <a href="{{ route('receptionist.notifications.show', $notif->id) }}"
                        class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors border border-transparent hover:border-emerald-100"
                        title="Xem chi tiết">
                        <span>Xem chi tiết</span> <i class="fa-solid fa-chevron-right text-[10px]"></i>
                    </a>
                    @endif
                    <form action="{{ route('receptionist.notifications.destroy', $notif->id) }}" method="POST"
