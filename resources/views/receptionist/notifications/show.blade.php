<x-layouts.receptionist title="Chi tiết thông báo">
    <div class="space-y-6">

        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                <div class="w-10 h-10 rounded-full {{ in_array($notification->type, ['cancellation', 'system_cancellation']) ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600' }} flex items-center justify-center">
            ...
            <a href="{{ route('receptionist.notifications.page') }}"
                class="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-emerald-600 bg-gray-50 hover:bg-emerald-50 px-4 py-2.5 rounded-lg transition-colors border border-gray-200 w-fit">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 md:p-8 border-b border-gray-100">
                ...
                <div class="flex items-center gap-3 text-sm font-medium text-gray-500">
                    ...
                    @endphp
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ in_array($notification->type, ['cancellation', 'system_cancellation']) ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800' }}">
                        {{ $typeLabel }}
                    </span>
                </div>
            </div>
            ...
            <div class="p-6 md:p-8 border-t border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm md:text-base">
                    ...
                    <div class="flex flex-col border-b border-gray-100 pb-3 md:border-0 md:pb-0">
                        <span class="text-gray-400 font-bold uppercase tracking-wider text-xs mb-1">Bác sĩ</span>
                        <strong class="text-gray-800">{{ $appointment->doctorProfile->full_title ?? 'Chưa xác định' }}</strong>
                    </div>
                    <div class="flex flex-col border-b border-gray-100 pb-3 md:border-0 md:pb-0">
                        <span class="text-gray-400 font-bold uppercase tracking-wider text-xs mb-1">Thời gian</span>
                        <strong class="text-gray-800 text-lg text-emerald-600">{{ \Carbon\Carbon::parse($appointment->appointment_time)->format('H:i') }} <span class="text-gray-500 text-base font-medium">- {{ $appointment->appointment_date->format('d/m/Y') }}</span></strong>
                    </div>
                    <div class="flex flex-col border-b border-gray-100 pb-3 md:border-0 md:pb-0">
                ...
                <div class="mt-6 pt-6 border-t border-gray-100">
                    <a href="{{ route('receptionist.appointments.show', $appointment->id) }}"
                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white font-semibold rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">
