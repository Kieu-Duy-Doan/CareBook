    ...
    @if(count($alternativeDoctors) > 0)
        ...
        @foreach($alternativeDoctors as $doc)
            <div style="background-color: #ffffff; border: 1px solid #e2e8f0; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
                <p style="margin: 0 0 8px 0; font-size: 18px; font-weight: bold; color: #0f172a;">{{ $doc->full_title }}</p>
                <p style="margin: 0 0 16px 0; font-size: 14px; color: #64748b;">Có lịch vào ngày: <strong>{{ \Carbon\Carbon::parse($doc->alternative_date)->format('d/m/Y') }}</strong></p>
                
                {{-- URL này sẽ dẫn trực tiếp vào trang đặt lịch với tham số doctor_id và date --}}
                <a href="{{ url('/dat-lich?doctor_id=' . $doc->id) }}" style="display: inline-block; background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 10px 20px; font-size: 16px; font-weight: bold; border-radius: 6px;">
