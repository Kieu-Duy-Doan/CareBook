<x-layouts.admin title="Quản lý Khách hàng">
    <div class="space-y-6">
        <!-- Action Bar -->
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="text-2xl font-bold text-gray-800">Quản lý Khách hàng</h2>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.customers.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center gap-2">
                    <i class="fa-solid fa-plus"></i> Thêm khách hàng
                </a>
                <a href="{{ route('admin.customers.export', request()->all()) }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition flex items-center gap-2">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            <form action="{{ route('admin.customers.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    @if(request()->anyFilled(['search', 'has_insurance', 'status']))
                        <a href="{{ route('admin.customers.index') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 flex items-center justify-center transition">
                            Đặt lại
                        </a>
                    @endif
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($customers as $key => $customer)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-center">{{ $customers->firstItem() + $key }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm bg-green-100 text-green-600 shrink-0">
                                            {{ $customer->avatar_initials ?? mb_substr($customer->full_name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="font-bold text-gray-900">{{ $customer->full_name }}</div>
                                            <div class="text-xs text-gray-500 mt-0.5">
                                                <i class="fa-regular fa-id-card mr-1"></i>{{ $customer->id_card ?? '—' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 font-medium">
                                    <i class="fa-solid fa-phone text-gray-400 mr-1.5 text-xs"></i>{{ $customer->phone }}
                                </td>
                                <td class="px-4 py-3">
                                    {{ $customer->email ?? '—' }}
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.customers.show', $customer->id) }}" class="w-8 h-8 rounded bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition" title="Xem chi tiết">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.customers.edit', $customer->id) }}" class="w-8 h-8 rounded bg-yellow-50 text-yellow-600 flex items-center justify-center hover:bg-yellow-100 transition" title="Chỉnh sửa">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <form action="{{ route('admin.customers.destroy', $customer->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xoá khách hàng này? Tất cả hồ sơ và dữ liệu liên quan sẽ bị xoá vĩnh viễn!');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-8 h-8 rounded bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-100 transition" title="Xoá khách hàng">
                            ...
                            <tr>
                                <td colspan="8" class="px-4 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-medium text-gray-600">Chưa có khách hàng nào.</p>
                                        <p class="text-sm text-gray-400 mt-1">Bấm vào "Thêm khách hàng" để tạo hồ sơ mới.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
