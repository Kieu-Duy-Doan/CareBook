                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">
                                    {{ $log->changedBy->name ?? 'Hệ thống / Vô danh' }}</div>
                                @if ($log->changedBy)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if (strtolower($log->action) == 'created' || strtolower($log->action) == 'tạo mới')
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200">Tạo
                                        mới</span>
                                @elseif(strtolower($log->action) == 'updated' || strtolower($log->action) == 'cập nhật')
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">Cập
                                        nhật</span>
                                @elseif(strtolower($log->action) == 'cancelled' || strtolower($log->action) == 'hủy')
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200">Hủy
                                        bỏ</span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">{{ $log->action }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                @if ($log->old_status || $log->new_status)
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="px-2 py-0.5 rounded bg-gray-100 text-gray-600 text-xs border border-gray-200">{{ $log->old_status ?? 'N/A' }}</span>
                                        <i class="fa-solid fa-arrow-right text-gray-400 text-xs"></i>
                                        <span
                                            class="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-xs border border-blue-200">{{ $log->new_status ?? 'N/A' }}</span>
                                    </div>
                                @else
