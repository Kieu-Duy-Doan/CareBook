<x-layouts.admin title="Chi tiết lịch hẹn #{{ $appointment->appointment_code }}">
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="w-full lg:w-2/3" x-data="{ activeTab: 'overview' }" @beforeprint.window="activeTab = 'prescription'">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6 print:hidden">
                <div class="flex border-b border-gray-100">
                    ...
                    <button type="button" @click="activeTab = 'history'"
                        ...
                        class="flex-1 py-4 px-2 sm:px-4 text-sm font-bold transition-colors border-r border-gray-100">
                    ...
                    <button type="button" @click="activeTab = 'payment'"
                        :class="{ 'border-b-2 border-blue-500 text-blue-600 bg-blue-50': activeTab === 'payment', 'text-gray-500 hover:text-gray-700 hover:bg-gray-50': activeTab !== 'payment' }"
                        class="flex-1 py-4 px-2 sm:px-4 text-sm font-bold transition-colors">
                        <i class="fa-solid fa-credit-card mr-1 sm:mr-2"></i><span class="hidden sm:inline">Thanh toán</span>
                    </button>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'payment'" class="space-y-6 print:hidden"
                x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between sticky top-0 z-10">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-base font-bold text-gray-900">Chi tiết Thanh toán</h3>
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div class="flex items-start gap-3">
                                    <div class="mt-1">
                                        @if($payment->method === 'cash')
                                            <i class="fa-solid fa-money-bill-wave text-green-600 text-xl"></i>
                                        @elseif($payment->method === 'transfer' || $payment->method === 'sepay')
                                    ...
                                    <div>
                                        <div class="font-bold text-gray-900">{{ number_format($payment->amount, 0, ',', '.') }} ₫</div>
                                        <div class="text-sm text-gray-600 mt-1">{{ $payment->description ?? 'Thanh toán dịch vụ' }}</div>
                                        <div class="text-xs text-gray-500 mt-1 flex items-center gap-2">
                                            <span><i class="fa-regular fa-clock mr-1"></i> {{ $payment->created_at->format('H:i d/m/Y') }}</span>
                                            <span>&bull;</span>
                                            <span class="capitalize">{{ $payment->method === 'cash' ? 'Tiền mặt' : ($payment->method === 'transfer' || $payment->method === 'sepay' ? 'Chuyển khoản' : ($payment->method ?? 'Không xác định')) }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    @endif
                                    <div class="text-xs text-gray-500 mt-2">Mã GD: #{{ $payment->id }}</div>
                                </div>
                            </div>
                            @endforeach
                            <div class="mt-4 pt-4 border-t border-gray-200 flex justify-end">
                                <div class="text-base text-gray-600 mr-4">Tổng đã thanh toán:</div>
                                <div class="text-lg font-bold text-gray-900">{{ number_format($appointment->payments->where('status', 'completed')->sum('amount'), 0, ',', '.') }} ₫</div>
                            </div>
                        </div>
                        @endif
                    </div>
