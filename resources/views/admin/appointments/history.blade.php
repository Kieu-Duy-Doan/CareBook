<x-layouts.admin title="Lịch sử khám bệnh viện">
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Lịch sử khám bệnh viện</h2>
            <p class="text-gray-500 mt-1">Danh sách toàn bộ lịch sử khám của bệnh viện</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.appointments.history.export-csv') }}?{{ http_build_query(request()->query()) }}"
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
        <form action="{{ route('admin.appointments.history') }}" method="GET" class="space-y-4">
            <!-- Hàng 1 -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.appointments.history') }}?clear_filter=1"
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
        <div class="text-sm text-gray-600 font-medium">
            Tìm thấy <span class="text-gray-900 font-bold">{{ $appointments->total() }}</span> lịch hẹn
        </div>
    </div>
                    ...
                    @forelse($appointments as $appt)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">
                                {{ number_format($appt->payments->sum('amount'), 0, ',', '.') }} ₫
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                @if($appt->clinicalVisits->count() > 0)
                                    @foreach($appt->clinicalVisits as $visit)
                                        <div class="mb-1">
                                            @if($visit->diagnosis)
                                                <strong>CĐ:</strong> {{ $visit->diagnosis }}<br>
                                            @endif
                                            @if($visit->clinical_notes)
                                                <span class="text-gray-500">{{ $visit->clinical_notes }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                @else
